...
void push_shack(CPUState *env, TCGv_ptr cpu_env, target_ulong next_eip)
{
    ...
    if (tb != NULL) {
      ...
      */
      ...
      tcg_gen_ld_ptr(shack_end_ptr, cpu_env, offsetof(CPUState, shack));
      tcg_gen_brcond_ptr(TCG_COND_NE, shack_top_ptr, shack_end_ptr, lbl_push);
      //flush shadow stack
      tcg_gen_ld_ptr(shack_top_ptr, cpu_env, offsetof(CPUState, shack));
      tcg_gen_add_ptr(shack_top_ptr, shack_top_ptr,
      ...
      */
      ...
      tcg_gen_ld_ptr(shack_top_ptr, cpu_env, offsetof(CPUState, shack_top));
      tcg_gen_add_ptr(shack_top_ptr, shack_top_ptr, tcg_const_tl(-sizeof(target_ulong)));
      tcg_gen_st_ptr(shack_top_ptr, cpu_env, offsetof(CPUState, shack_top));
      tcg_gen_st_ptr(tcg_const_ptr(next_eip), shack_top_ptr, 0);
      tcg_gen_st_ptr(tcg_const_ptr((unsigned long)host_pc), shack_top_ptr, stk_diff);
...
{
    TCGv_ptr guest_eip = tcg_temp_new_ptr(), temp_eip = tcg_temp_local_new();
    int lbl_else = gen_new_label(), lbl_cache = gen_new_label();
    ...
    */
    ...
    tcg_gen_mov_tl(temp_eip, next_eip);
    tcg_gen_brcond_ptr(TCG_COND_NE, guest_eip, next_eip, lbl_cache);
    /* reload `shack_top_ptr`: It's a temporary (not a "local" temporary),
     * hence is dead after the branch `brcond`
     */
    tcg_gen_ld_ptr(shack_top_ptr, cpu_env, offsetof(CPUState, shack_top));
    tcg_gen_ld_ptr(temp_eip, shack_top_ptr, stk_diff);
    tcg_gen_add_ptr(shack_top_ptr, shack_top_ptr, tcg_const_tl(sizeof(target_ulong)));
    tcg_gen_st_ptr(shack_top_ptr, cpu_env, offsetof(CPUState, shack_top));
    *gen_opc_ptr++ = INDEX_op_jmp;
    *gen_opparam_ptr++ = GET_TCGV_I32(temp_eip);
    /*
      } else {
        next_eip = temp_eip
        target_ulong idx = next_eip & CALL_CACHE_MASK;
